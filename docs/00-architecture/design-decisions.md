...
Phase 7 revision: dropping MISP from the SOC platform

When Phase 7 was first scoped, the response stack was going to be **TheHive + Cortex + MISP. After deploying TheHive and Cortex, I reconsidered MISP and decided **not** to include it. This is the reasoning, kept here because the decision is more useful documented than silently made.
...
- **A SOC analyst's job doesn't end at the SIEM dashboard.** In a production SOC, a relevant alert triggers case creation, gets enriched with WHOIS / VirusTotal, and is worked through to closure. Documenting only the detection side leaves the L1 role portrait incomplete. Adding TheHive + Cortex reproduces the flow that a real L1 executes every day.
