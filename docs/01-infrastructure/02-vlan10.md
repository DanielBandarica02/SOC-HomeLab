# Phase 1 — Part 2: VLAN 10 (Corporate Environment)
...
The `SOC-10-AD` VM was created in VirtualBox with one virtual NIC attached to the `internal-vlan10-corp` Internal Network. The intent is that the DC is reachable only from inside VLAN 10 — there is no admin shortcut from the host or from any other VLAN.
...
| RAM      | 4 GB |
| Disk     | 60 GB |
| NIC 1    | Internal Network `internal-vlan10-corp`, Promiscuous Allow All |
...
*Previous: [Phase 1 — Part 1: Network Backbone (pfSense + OpenVPN)](01-pfsense.md)*
*Next: [Phase 1 — Part 3: VLAN 20 (Software Development)](03-vlan20.md)*
