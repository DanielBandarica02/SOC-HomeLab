...
During initial setup, alerts generated by Wazuh (specifically lower-level alerts like Suricata's level 3 network events) were not appearing in Splunk Search despite the HEC token being correctly configured. The root cause was twofold: 

1. **Syntax Placement:** The `<integration>` block in `ossec.conf` was mistakenly placed inside a duplicate `<ossec_config>` tag that Wazuh ignored.
2. **Alert Filtering Threshold:** By default, Wazuh integration engines apply a filtering threshold. Low-level logs were discarded before leaving the manager.

**Solution:** Ensured the `<integration>` block was placed inside the primary, single valid `<ossec_config>` block, and explicitly forced the engine to forward all telemetry by setting `<level>0</level>` within the integration parameters. 

```xml
<integration>
  <name>custom-splunk-hec</name>
  <hook_url>[http://192.168.10.40:8088/services/collector/event](http://192.168.10.40:8088/services/collector/event)</hook_url>
  <api_key><your_hec_token></api_key>
  <alert_format>json</alert_format>
  <level>0</level>  </integration>
```
