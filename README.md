...
Extension of the SIEM-only environment into a complete SOC platform combining network intrusion detection and response tooling. TheHive deployed as case management platform, with Cortex as analysis engine and MISP as threat intelligence platform, all containerized via Docker Compose on a dedicated `soc-platform` VM (10.10.99.20) under the SOC Management VLAN. 
